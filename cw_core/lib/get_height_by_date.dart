...
// FIXME: Hardcoded values; Works only for xck
// Update periodically using actual chain heights.
...
int getMoneroHeigthByDate({required DateTime date}) {
  ...
  final values = dates.values.toList();
  final keys = dates.keys.toList();
  ...
  try {
    if (dates[raw] == null || dates[raw] == values.last) {
      final startHeight = values.length > 1 ? values[values.length - 2] : values.last;
      final endHeight = values.last;

      final endDateRaw = keys.last.split('-');
      final endDate = DateTime(
        int.parse(endDateRaw[0]),
        int.parse(endDateRaw[1]),
      );

      final daysInMonth = DateTime(endDate.year, endDate.month + 1, 0).day;
      final heightPerDay = (endHeight - startHeight) / daysInMonth;

      final differenceInDays = date.difference(endDate).inDays;
      height = endHeight + (differenceInDays * heightPerDay).round();
    } else {
      final startHeight = dates[raw]!;
      final index = values.indexOf(startHeight);
      final endHeight = values[index + 1];

      final startDateRaw = keys[index].split('-');
      final startDate = DateTime(
        int.parse(startDateRaw[0]),
        int.parse(startDateRaw[1]),
      );
      ...
      height = startHeight + daysHeight.round();
    }
  } catch (e) {
    printV(e.toString());
  }
